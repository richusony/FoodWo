...
async function signInUser(req, res) {
  ...
  if (!fullname || !email || !phone || !address || !password) {
    ...
    return res
      .status(401)
      .json({ failed: "User already exists with this email" });
  } else if (userExistsPhone) {
    return res
      .status(401)
      .json({ failed: "User already exists with this phone no." });
  } else {
    try {
  ...
  if (userInputOtp === storedOtp) {
    ...
    newUser
      ...
      .then(async () => {
        ...
        const addingAddress = await addressModel.create({
          userId: newUser._id,
          address1: address,
          address2: false,
          address3: false,
        });
        const creatingZeroWallet = await walletModel.create({
          userId: newUser._id,
          balance: 0,
        });
        ...
        if (findCode) {
          ...
          const addToWallet = await walletModel.updateOne(
            { userId: codeOwner },
            {
              $inc: { balance: 100 },
          ...
          const addNewUserToReferModel = await referModel.updateOne(
            { referalCode: referCode },
            { $push: { usedUsers: newUser._id } }
          );
        }
...
async function addToCart(req, res) {
  ...
  if (exists) {
    ...
    const adding = await cartModel.create({
      userId: userid,
      foodId: foodid,
      quantity: 1,
    });
...
async function removeFromCart(req, res) {
  ...
  const deleting = await cartModel.deleteOne({
    userId: userid,
    foodId: foodid,
  });
  if (deleting) {
...
async function addToWishlist(req, res) {
  ...
  if (exists) {
    const deleting = await wishListModel.deleteOne({
      foodId: foodid,
      userId: userid,
    });
    res.status(200).json({ added: false });
  } else {
    const adding = await wishListModel.create({
      foodId: foodid,
      userId: userid,
    });
    if (adding) {
...
async function updateUserProfile(req, res) {
  const { userid, fullname, email, phone, address1, address2, address3 } =
    req.body;
  ...
  if (!address1 && !address3) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address2, address2: false, address3: false }
    );
  } else if (!address2 && !address3) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address1, address2: false, address3: false }
    );
  } else if (!address1 && !address2) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address3, address2: false, address3: false }
    );
  } else if (!address1) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address2, address2: address3, address3: false }
    );
  } else if (!address2) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address1, address2: address3, address3: false }
    );
  } else if (!address3) {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
      { address1: address1, address2: address2, address3: false }
    );
  } else {
    const updateAddress = await addressModel.updateOne(
      { userId: userid },
...
async function addNewAddress(req, res) {
  ...
  if (aid == 1) {
    const updating = await addressModel.updateOne(
      { userId: uid },
      { address1: newaddress }
    );
    if (updating) {
...
async function updateUserAddress(req, res) {
  ...
  if (aid == 1) {
    if (newaddress === "") {
      ...
      const updating = await addressModel.updateOne(
        { userId: uid },
        { address1: oldAddress2, address2: oldAddress3, address3: false }
      );
      if (updating) {
        res.status(200).json({ updated: true });
      } else {
    ...
    if (newaddress === "") {
      const updating = await addressModel.updateOne(
        { userId: uid },
        { address2: oldAddress3, address3: false }
      );
      if (updating) {
        res.status(200).json({ updated: true });
      } else {
  ...
  if (product) {
    if (parseInt(productQty) > parseInt(product.productInStock)) {
      return res
        .status(400)
        .json({
          err: `only ${product.productInStock} left for ${product.productName}`,
        });
    }
  }
...
async function updateStock(req, res) {
  ...
  if (parseInt(productQty) > parseInt(product.productInStock)) {
    return res
      .status(400)
      .json({ err: `only ${product.productInStock} left for ${productName}` });
  }
  ...
  if (coupon != undefined && coupon != "") {
    if (existCoupon) {
      if (existCoupon.usedUsersCount >= existCoupon.usersLimit) {
        return res
          .status(400)
          .json({ err: "Coupon has been reached the maximum users limit" });
      }
      if (checkFood) {
        if (existCoupon.status === "deactive") {
          ...
          const usedOrNot =
            userData.usedCoupons.length == 0
              ? false
              : userData?.usedCoupons.filter(
                  (coup) => coup && coup.couponId == existCoupon._id.toString()
                );
          ...
          if (usedOrNot == false || usedOrNot[0] == false) {
            ...
            if (endDate < currentDate) {
              ...
              const updatingUser = await userModel.updateOne(
                { _id: user_id },
                { $push: { usedCoupons: updateData } }
              );
              if (updatingUser) {
                const updateCoupon = await couponModel.updateOne(
                  { _id: existCoupon._id.toString() },
                  { $inc: { usedUsersCount: 1 } }
                );
                console.log(updateCoupon);
                if (existCoupon.discountType == "per") {
                  afterDiscountPrice =
                    parseInt(productPrice * productQty) -
                    (existCoupon.discountValue * parseInt(productPrice)) / 100;
                  discountAmount =
                    (existCoupon.discountValue * parseInt(productPrice)) / 100;
                  ...
                  afterDiscountPrice =
                    parseInt(productQty) * parseInt(productPrice) -
                    existCoupon.discountValue;
                  discountAmount = existCoupon.discountValue;
                  console.log("after discount : : : : : ", afterDiscountPrice);
                  discountAddedOrNot = true;
                }
                console.log(
                  "usedCoupons after update: ",
                  userModel.usedCoupons
                );
              } else {
                return res
                  .status(500)
                  .json({
                    added: false,
                    err: "Database is having some issues",
                  });
              }
            }
          } else {
            const findCoupon = usedOrNot.filter(
              (coup) => coup && coup.couponId == existCoupon._id.toString()
            );
            ...
            if (usedCount >= usageLimit) {
              return res
                .status(400)
                .json({ added: false, err: "reached coupon limit." });
            } else {
              const updating = await userModel.updateOne(
                {
                  _id: user_id,
                  "usedCoupons.couponId": findCoupon[0].couponId,
                },
  ...
  if (productOffer) {
    if (productOffer.discountType == "per") {
      offerDiscount = parseInt(
        (productOffer.discountValue * productOffer.actualPrice) / 100
      );
    } else {
  ...
  try {
    ...
    if (product.productInStock <= 0) {
      ...
      const addToOrder = await orderModel.create({
        ...
        productImage: image,
        productPrice: afterDiscountPrice - offerDiscount,
        address: address,
      ...
      if (existCoupon && existCoupon.foodId == productId) {
        if (paymentMethod == "FoodWo Wallet") {
          ...
          const calculatedAmount =
            parseInt(productPrice) * parseInt(productQty) - productPrice;
          console.log("Calculated.... : : ", calculatedAmount);
          console.log(
            "Reduced.... : : ",
            calculatedAmount + afterDiscountPrice
          );
        ...
        if (paymentMethod == "FoodWo Wallet") {
          ...
          const walletUpdate = await walletModel.updateOne(
            { userId: user_id },
            {
              $inc: {
                balance: -(
                  parseInt(productPrice) * parseInt(productQty) -
                  offerDiscount
                ),
      ...
      if (result) {
        ...
        const createInvoice = await invoiceModel.create({
          ...
          discount:
            offerDiscount == 0 ? discountAmount.toFixed(2) : offerDiscount,
          amount: afterDiscountPrice - offerDiscount,
          paymentMethod: paymentMethod,
        });
...
async function viewProductDetailsPage(req, res) {
  try {
    ...
    const fdReviews = await fdReviewModel
      .find({ foodId: id })
      .sort({ createdAt: -1 });
...
async function viewOrderItemPage(req, res) {
  try {
    ...
    res.render("../views/orderedItem.ejs", {
      order: orderedItem,
      user: userDetails,
    });
  } catch (err) {
...
async function cancelOrder(req, res) {
  ...
  if (orderId) {
    const updating = await orderModel.updateOne(
      { orderId: orderId },
      { orderStatus: "Cancelled" }
    );
    if (updating) {
      const updateProductQty = await productModel.updateOne(
        { _id: productId },
        { $inc: { productInStock: purchasedQty, productSold: -purchasedQty } }
      );
      const currentDate = moment().format("DD-MM-YYYY"); // Get the current date in 'DD-MM-YYYY' format
      const historyData = {
        date: currentDate,
      ...
      const refundToWallet = await walletModel.updateOne(
        { userId: userId },
        {
          $inc: { balance: purchasedPrice * purchasedQty },
          $push: { history: historyData },
        }
      );
      if (refundToWallet) {
